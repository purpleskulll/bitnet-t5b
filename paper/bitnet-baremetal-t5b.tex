...
\code{bitnet.cpp}, an interleave that is \code{i2\_s}'s at two bits and
\code{TQ1\_0}'s at \num{1.600}, adopted rather than designed, so the access
pattern is unchanged from either, a decode of four \emph{independent} magic multiplies, a
...
digits, in the same file the open question of \S\ref{sec:contrib} comes from:
