...
\item A decode of depth one rather than depth five (\S\ref{sec:kernel}). That a
      ...
      \textbf{We answered that question, and the answer is no --- for
      \code{TQ1\_0}.} Transplanting the depth-one form into upstream's own
      kernel and measuring it there gives $\num{0.983}\times$, slower in 7 of
      ...
      \textbf{And against the serial form \emph{this} encoding admits, depth
...
\textbf{And it has now been run}~(\code{results/insitu\_symmetric.txt}). The measurement needed a patched
...
\paragraph{Why \SI{19.6}{\percent} fewer weight bytes give \SI{14}{\percent}
...
decorative. Matmuls are \SI{49.5}{\percent} of a \emph{baseline} token; t5b
removes \SI{21.1}{\percent} of them, which is \SI{4.48}{\milli\second}, which is
\SI{10.5}{\percent} of the baseline token, which predicts $\num{1.117}$ end to
end. The same run's wall clock gives $\num{5.479}/\num{4.905}=\num{1.117}$. The
