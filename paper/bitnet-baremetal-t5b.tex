...
\code{i2\_s} path into dispatch, accumulator fold and per-column
post-processing --- one probe at one site yields one number, and separating the
three would need probes inside \code{tinyBLAS\_I2S\_AVX}. And it is one host:
the same shared machine as everything else in \S\ref{sec:results}.
