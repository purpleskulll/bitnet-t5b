...
\textbf{Two of the three candidates are ruled out.} The cost is the inner loop's
own arithmetic and memory traffic, not overhead around it, so the \num{1.257}
is a statement about the kernel rather than about \code{ggml}'s dispatch. The
third candidate, the 32-block accumulator fold, lies \emph{inside} the
