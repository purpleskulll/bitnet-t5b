...
results are from a single AVX2 microarchitecture \emph{without} VNNI. A static
pipeline model, calibrated against that host to \SI{3.0}{\percent}, finds the
arithmetic ratio stable across Zen 3/4/5, Ice Lake and Sapphire Rapids, so it is
not an artefact of one narrow part; the same model bounds the cost of VNNI at
\numrange{0}{14}\,\% but cannot measure it, and an independent system targeting
VNNI stores ternary weights at eight bits for that reason.
\end{abstract}
...
upstream's 32. \textbf{Twelve are killed.}

The remaining two survive, and checking rather than filing them is what makes
the result meaningful: both are \emph{exactly equivalent} on every reachable
...
any of the \num{65536} word lanes, because no low byte ever borrows --- the
invariant \S\ref{sec:lane} asserts, confirmed independently of the text that
...
\code{llama.cpp}'s \code{i2\_s} AVX2 kernel disagrees with exact arithmetic on
...
measured 200 of 200 on two tensors and wrote the prediction as an absolute; at
\num{12000} rows the tail is visible and the correct statement is statistical.
...
figure. The stated reason for instrumenting only one arm --- that the control
must stay unperturbed --- does not survive scrutiny either. The integration now
probes \textbf{both} arms at the same dispatch site in the same expression, so
the ratio becomes a quotient of two measured quantities and whatever the
dispatch costs is common to both and divides out.

The probe's cost was measured rather than assumed, and the assumption would have
been wrong instructively. An \code{rdtsc} pair alone costs \SI{19}{\nano\second}
...
\textbf{What this does not yet produce is a number.} Running it requires
rebuilding the patched \code{llama.cpp} image, which needs a Docker daemon this
...
enables is not claimed here, and the \num{1.606} above stands as the derived
quantity it is.
...
\SI{14}{\milli\second}, or \SI{30}{\percent}, and an earlier draft quoted that
number here. The gap is the replay's, not the model's: it drives
\code{bitnet\_vec\_dot\_i2\_i8\_s\_reference}, which \S\ref{sec:endtoend}
...
\emph{bounds} that move at \numrange{0}{14}\,\% but does not measure it.
\textbf{The central result should be read as a property of AVX2 without VNNI
until it is run on a VNNI part.} The code compiles for \code{-mavxvnni} here but
...
reported.
\item The negative control did not fire. \code{llvm-mca} 23.1.0 assembles and
...
\textbf{None of this is a measurement}, and for some time we claimed one was a
single command away when it was not. \code{second\_datapoint.sh} carried a
header saying a run on a VNNI part would settle the question. A reviewer ran it
on an Intel Xeon reporting \code{avx512\_vnni}, then checked the binaries with
\code{objdump}: \textbf{zero \code{vpdpbusd}}, with \code{-march=native} in the
flags. No compiler contracts \code{vpmaddubsw} followed by an accumulating
\code{vpaddw} into \code{VPDPBUSD} as an idiom --- the instruction must be
written, and it had not been. Every run of that script measured the AVX2 kernels
on whatever microarchitecture it was given.
...
\textbf{What is missing is therefore hardware access, and nothing else.} That
...
\code{int64} on every row, the harness refuses to report a number it did not
measure, and the benchmark needs no model, no container and no privileges. Every
machine available to this work reports \code{avx2 fma} and nothing further. One
run on a part with \code{avx512vnni} or \code{avx\_vnni} replaces the modelled
figures below with measured ones, and until that run exists we report the model
...
The same inequality predicts the result will narrow on a part with VNNI --- by
\numrange{0}{14}\,\% in the static model of \S\ref{sec:microarch}, though that
model's negative control failed and it is no substitute for the run --- and an
