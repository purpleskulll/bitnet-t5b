...
\S\ref{sec:contrib} concedes each piece to its owner. What is left: a block
...
$K=\num{6912}$ --- statistical rather than absolute, because with
\SI{0.78}{\percent} of headroom the outcome turns on each row's own code mean.
...
not survive and is withdrawn in \S\ref{sec:contrib}, along with the scale
...
remainder byte, the scale riding in \code{i2\_s}'s existing 32-byte tail so
...
\code{TQ1\_0} has no equivalent of --- and the \emph{measurement} of the
