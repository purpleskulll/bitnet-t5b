...
\item A \num{1.600}-bit variant of the \emph{known} base-3 packing
      ...
      than with the decode of \S\ref{sec:kernel}. Ours is the
...
\item A decode of depth one rather than depth five (\S\ref{sec:kernel}). That a
      ...
      $\lfloor x/3^{j}\rfloor$ \textbf{independently, straight from the original
      ...
      \textbf{That arithmetic is not ours either, and at exactly these
      ...
      contraction. \textbf{That distinction does not rescue a novelty claim
      ...
      16-bits?}, \code{arch/x86/quants.c:1406} --- and \S\ref{sec:kernel}
      answers it with a measurement. We verified every sentence of this
      paragraph against the vendored kernel at \code{390c307} rather than
      against a description of it.

      The depth-one extraction costs more multiply-port operations than the
      serial form and buys latency; \S\ref{sec:condition} gives the condition
...
\item A \emph{measurement} of the profitability condition
      (\S\ref{sec:condition}), derived from~\eqref{eq:roofline} and verified by
      measuring both $\pi$ and $B$ rather than inferring either. The criterion
      itself is twenty years old: Equation~1 of Zukowski et
      al.~\cite{zukowski} switches between the I/O-bound and CPU-bound regimes
...
\S\ref{sec:correct} belongs to, and sharpens why it happens. The kernel feeds
...
\textbf{The nine omitted folds cost \SI{7.5}{\percent} of the dispatched path's
...
(\code{-DTERNARY\_T5B\_SHUFDIG}) and not a second format. It is bit-identical:
the full suite of \num{97529} assertions passes against it, and the two decoders
...
\S\ref{sec:limits} is the latter count.} Fifteen interleaved rounds, with the
...
\emph{No cell of the surplus table changes.} \eqref{eq:condition} failed at one
...
the graph \code{llama.cpp} builds for this architecture is probably not the
model's. \code{build\_ffn} is called with \code{LLM\_FFN\_SILU} in
\code{src/models/bitnet.cpp}, whereas the reference implementation released with
the checkpoint applies a squared rectifier ---
\code{inner = self.ffn\_sub\_norm(squared\_relu(x1) * x3)}. The file does not
...
\code{src/llama-graph.h} and is used by five other architectures. We have not
rebuilt with it and therefore do not claim which activation the quoted
\num{96.8243} reflects; we claim only that the number has two independent
