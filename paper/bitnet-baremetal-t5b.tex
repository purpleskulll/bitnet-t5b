...
\item A \num{1.600}-bit variant of the \emph{known} base-3 packing
      ...
      \textbf{Nor is the scale model ours, and it is in the very pull request
      cited above.} \code{block\_q1\_3} lived on \#8151 from 2024-06-19 to
      ...
      \code{i2\_s}'s convention, not ours. As with the packing and the decode,
...
every $\Sigma$ below by a factor $1/\num{0.63}$ --- to \num{2.12} at one thread
and \num{3.39} at four --- which reverses the prediction at two and at four
...
under an upstream type number. Of the remaining fifth, \SI{78.2}{\percent} was
present in the same pull request as \code{Q1\_3} before \code{TQ1\_0} replaced
it (\S\ref{sec:contrib}). What is attributable here is \num{4546560} bytes ---
\SI{1.03}{\percent} of the \code{TQ1\_0} ternary payload and
\SI{0.38}{\percent} of the file --- and it is the block length, $160=32\times5$
